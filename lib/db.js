...
const { NetworkStatus } = require('bitspace-rpc/messages')

const INTERNAL_NAMESPACE = '@bitspace:internal'
const NETWORK_PREFIX = 'network'

module.exports = class BitspaceDb extends Nanoresource {
  ...
  async _open () {
    this._namespacedStore = this.chainstore.namespace(INTERNAL_NAMESPACE)
    await this._namespacedStore.ready()
    const dbFeed = this._namespacedStore.default()
    this._db = bittrie(null, null, { feed: dbFeed })
