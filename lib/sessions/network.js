...
module.exports = class NetworkSession {
  constructor (client, sessionState, chainstore, networker, db, networkStates, opts = {}) {
    this._client = client
  ...
  _setConfiguringTimeouts (discoveryKey) {
    // technically the chain can load *during* the configuration call as well
    // in practice that wont imply the pipelining guarantee so we dont need
    // to worry about it
    if (!this._chainstore.isLoaded({ discoveryKey })) return
    const chain = this._chainstore.get({ discoveryKey })
    const cbSet = new Set()
    const mainTimeouts = chain.timeouts
    let called = false
    chain.timeouts = {
      get: (cb) => {
